...
  // The specific paperId we *just* submitted (can be null/undefined on direct navigation)
...
  // 🚨 State to hold ALL fetched results (history)
...
        // 1. Fetch ALL History for the student
        try {
            const historyResponse = await fetch(`${API_BASE_URL}/api/student/results/history?studentId=${studentId}`);
...
        }
        ...
        if (fetchedHistory.length > 0) {
            ...
            if (currentPaperIdFromUrl) {
                targetResult = fetchedHistory.find(r => r.id === parseInt(currentPaperIdFromUrl));
            }
...
        <CheckCircle size={24} className="text-blue-600 mr-2 animate-spin"/> Calculating Results...
      </div>
    );
  }
...
        <p className='mt-2'>Please complete an exam and ensure the administrative scoring is run.</p>
      </div>
    );
  }
...
                  <span className="text-gray-500">80</span><span className="text-gray-500">40</span><span className="text-gray-500">20</span><span className="text-gray-500">0</span>
                </div>
...
                <Bar subject="Maths" score={currentExamData.mathsScore} maxScore={80} color="bg-blue-600" />
                <Bar subject="Physics" score={currentExamData.physicsScore} maxScore={40} color="bg-orange-500" />
                <Bar subject="Chemistry" score={currentExamData.chemistryScore} maxScore={40} color="bg-green-600" />
            </div>
          </div>
...
                ))}
                ...
                {allResultsHistory.length === 0 && !isLoading && !error && (
                    <p className="text-gray-500 p-4 bg-white rounded-lg border">No past completed exams found.</p>
                )}
            </div>
          </div>
  ...
  return (
    ...
    <div className="flex flex-col items-center w-24 h-full pt-2">
      ...
      <div className="flex-1 flex items-end w-16">
        ...
        <span className="absolute -top-6 text-sm font-bold text-gray-800">{score}</span>
