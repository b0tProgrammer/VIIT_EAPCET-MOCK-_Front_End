...
function DashboardCountdown({ nextMockTest, nextExamStartTime, onStart }) {
  ...
  useEffect(() => {
    ...
    const startWindowEnd =
      examStartTime + LATE_START_WINDOW_MINUTES * 60 * 1000;
    ...
    <div className="bg-blue-50 bg-opacity-80 p-6 rounded-xl shadow-md border border-blue-200">
      ...
      <div className="mt-2 text-sm text-gray-600">
        {nextMockTest?.durationHours && (
          <div>
            Duration: {nextMockTest.durationHours} hour
            {nextMockTest.durationHours > 1 ? "s" : ""}
          </div>
        )}
        ...
        {nextMockTest?.startTime && (
          <div className="text-xs text-gray-500 mt-1">
            Scheduled:{" "}
            {new Date(
              new Date(nextMockTest.startTime).getTime() - 5.5 * 60 * 60 * 1000,
            ).toLocaleString([], {
              month: "short",
              day: "numeric",
              hour: "2-digit",
        ...
        {status === "live" && (
          <div className="inline-block mt-2 px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">
            Active
          </div>
        )}
      </div>
  ...
  const performancePoints = useMemo(
    () =>
      history.map((h, i) => ({
        ...
        label:
          h.title || h.testName || h.paperTitle || h.name || `Test ${i + 1}`,
      })),
    [history],
  );
    ...
    const fetchData = async () => {
      try {
        const [examsRes, historyRes] = await Promise.all([
          fetch(`${API}/api/student/exams`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch(
            `${API}/api/student/results/history?studentId=${localStorage.getItem("studentId")}`,
            { headers: { Authorization: `Bearer ${token}` } },
          ),
        ]);
        if (examsRes.ok) {
        ...
        if (historyRes.ok) {
          ...
          if (hist.length > 0) {
            const passed = hist.filter(
              (h) => Number(h.score) / Number(h.totalMarks) >= 0.5,
            ).length;
    ...
    <>
      ...
      <div className="flex min-h-screen">
        ...
        <main className="flex-1 p-6 bg-gray-50 relative">
          ...
                   {" "}
          <p className="text-lg text-gray-600 mb-8">See your Performance here...</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
            ...
            <div className="bg-blue-50 bg-opacity-80 p-6 rounded-xl shadow-md border border-blue-200 flex flex-col justify-center items-center h-40 text-center">
              <p className="text-5xl font-bold text-gray-800 mb-2">{examsWritten}</p>
              <p className="text-lg text-gray-600">Overall Exams Written</p>
            </div>

            <div className="bg-blue-50 bg-opacity-80 p-6 rounded-xl shadow-md border border-blue-200 flex flex-col justify-center items-center h-40 text-center">
              <p className="text-5xl font-bold text-gray-800 mb-2">{passPercentage}</p>
              <p className="text-lg text-gray-600">Pass Percentage</p>
            </div>
          </div>
                   {" "}
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
            ...
            <div className="w-full h-80 bg-white border border-gray-200 rounded-lg px-4 py-6">
              <div className="h-full flex items-end gap-6 overflow-x-auto pb-2">
                ...
                {performancePoints.map((p, idx) => {
                  const rawPct =
                    ((Number(p.score) || 0) / (Number(p.total) || 1)) * 100;
